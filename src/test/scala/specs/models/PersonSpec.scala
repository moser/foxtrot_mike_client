...
import dispatch.json.{ JsObject, JsString, JsNumber, JsFalse }
...
class PersonSpec extends Spec with ShouldMatchers with BeforeAndAfterAll {
  describe("person") {
    it("should have a constructor accepting a JsObject") {
      val o = new JsObject(Map(JsString("id") -> JsString("123.1"), JsString("firstname") -> JsString("lala"), JsString("lastname") -> JsString("DDD"), JsString("group_id") -> JsNumber(0), JsString("disabled") -> JsFalse))
      val p = new Person(o)
